...
- Hybrid render gate (M3.5-1): `tests/e2e/test_hybrid_render.py` drives the full deployed stack over HTTP (nginx-cache → renderer → Netbox) — the seam the in-process M2.5-1 gate can't reach. Asserts the cold path (MISS, full render, byte-equal to a direct renderer fetch), the warm path (HIT served from disk in < 50 ms), the M2-6 metrics across the cache tier (cold increments `host_config_renders_total{role,outcome=success}`; warm does NOT — proving nginx serves it without touching the renderer), `X-Request-Id` correlation, and Netbox-down resilience (with the Netbox container stopped, an already-cached key still serves 200 so in-flight boots continue, while a cold key fails fast with 502/503/504 — bounded blast radius; always restarts Netbox). Lives under `tests/e2e/` so `just lab-test` runs it, but needs no KVM. Validated live on a DO Droplet (6/6). **M3.5 milestone closed.** (M3.5-1, #22)
